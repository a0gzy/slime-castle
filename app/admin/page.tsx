...
export default function AdminPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Fetch languages (assuming collection 'languages')
            ...
            // If collection is empty, save AND use default languages
    ...
    const handleSaveUser = async (updatedUser: AdminUser) => {
        try {
            ...
            alert(t('profile.save_drive_success')); // simple success alert
        } catch (error) {
    ...
    if (authLoading || (loading && isAdmin)) {
        return <div className="flex-1 flex items-center justify-center bg-zinc-950 text-zinc-400">Loading admin panel...</div>;
    }

    if (!isAdmin) return null; // Should be handled by router.push, but as a safeguard
        ...
        <div className="flex-1 flex flex-col p-6 md:p-12 bg-zinc-950 text-zinc-300">
            <div className="max-w-5xl w-full mx-auto space-y-12">
                ...
                <div>
                    ...
                    <p className="text-zinc-500">Manage users, roles, and supported languages.</p>
                </div>
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-xl">
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex gap-3 mt-2 items-center">
                            <input
                                type="text"
                                placeholder="ID (e.g. es)"
                                value={newLangId}
                                onChange={e => setNewLangId(e.target.value)}
                                className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-zinc-100 focus:border-purple-500 outline-none w-24"
                            />
                            <input
                                type="text"
                                ...
                                className="flex-1 max-w-xs bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-zinc-100 focus:border-purple-500 outline-none"
                            />
                            <button
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        ...
                        <div className="relative w-full sm:w-auto">
                            ...
                            <input
                                ...
                                className="w-full sm:w-64 bg-zinc-900 border border-zinc-800 rounded-lg pl-9 pr-4 py-2 text-sm text-zinc-100 focus:border-blue-500 outline-none"
                            />
                        </div>
                    </div>
...
function UserCard({ user, languages, onSave, t }: { user: AdminUser, languages: AppLanguage[], onSave: (u: AdminUser) => void, t: any }) {
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3 mt-2">
                <div className="flex flex-col gap-1">
                    <label className="text-xs font-bold uppercase tracking-wider text-zinc-500">{t('admin.role')}</label>
                    <select
                        value={role}
                        onChange={e => setRole(e.target.value as any)}
                        className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-300 outline-none focus:border-blue-500"
                    >
                ...
                {(role === 'editor' || role === 'admin') && (
                    <div className="flex flex-col gap-1">
                        <label className="text-xs font-bold uppercase tracking-wider text-zinc-500">{t('admin.languages')}</label>
                        <div className="flex flex-wrap gap-2">
                            {languages.map(lang => (
                                <button
                                    key={lang.id}
                                    onClick={() => toggleLang(lang.id)}
                                    className={`px-2 py-1 rounded text-xs font-bold transition-colors border ${selectedLangs.includes(lang.id)
            ...
            <button
                onClick={handleSave}
                disabled={!hasChanges}
                className="mt-2 flex items-center justify-center gap-2 w-full py-2.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-bold disabled:opacity-50 disabled:bg-zinc-800 disabled:text-zinc-500 transition-colors"
            >
